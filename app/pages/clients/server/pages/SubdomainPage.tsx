// filepath: e:/hightpanel/app/pages/clients/server/pages/SubdomainPage.tsx
'use client';
...
interface DomainOption { value: string; label: string }
interface RecordRowProps { label: string; value: string | number; copy?: boolean }
const RecordRow: React.FC<RecordRowProps> = ({ label, value, copy }) => (
    <div className="flex items-start justify-between gap-4">
        <div className="flex flex-col">
            <span className="text-[10px] font-semibold text-zinc-500 tracking-wider">{label.toUpperCase()}</span>
            <span className="text-zinc-300 font-mono text-xs break-all">{String(value)}</span>
        </div>
        {copy && (
            <button
                type="button"
                onClick={() => navigator.clipboard.writeText(String(value))}
                className="px-2 py-1 rounded-md bg-zinc-800 hover:bg-zinc-700 text-[10px] text-zinc-300 font-medium transition"
                title="Copiar"
            >Copiar</button>
        )}
    </div>
...
const SubdomainPage: React.FC = () => {
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            try {
                ...
                if (!cancelled) {
                    setError(e.message || 'Falha ao carregar domínios.');
                    addToast(e.message || 'Falha ao carregar domínios.', 'error');
                }
    ...
    const canSubmit = !!server?.id && !hasSubdomain && !!selectedDomainId && validateSub(subPart) && !formLoading;
    ...
    if (server && hasSubdomain) {
        ...
        const ipContent = server.primaryAllocation?.externalIp || server.primaryAllocation?.ip || (server.ip?.split(':')[0]) || '0.0.0.0';
        const port = server.primaryAllocation?.port || (server.ip?.split(':')[1] ? parseInt(server.ip.split(':')[1]) : undefined) || 25565;
        const aRecord = { type: 'A', name: fqdn, content: ipContent, ttl: 120, proxied: false };
        const srvRecord = { type: 'SRV', name: `_minecraft._tcp.${fqdn}`, data: { service: '_minecraft', proto: '_tcp', name: fqdn, priority: 1, weight: 1, port, target: fqdn } };

        return (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.25 }} className="flex flex-col gap-6">
                <ConfirmModal
                ...
                <div className="flex items-center justify-between flex-wrap gap-4">
                    <div>
                        <h2 className="text-lg font-semibold">Subdomínio</h2>
                        <p className="text-sm text-zinc-400">Gerencie o subdomínio criado para este servidor.</p>
                    </div>
                    <div className="flex gap-3">
                        <button
                            ...
                            className="px-4 py-2 rounded-lg bg-zinc-800 text-zinc-200 text-sm font-medium hover:bg-zinc-700 transition"
                        >Copiar FQDN</button>
                        <button
                            onClick={() => setShowDeleteModal(true)}
                            disabled={formLoading}
                            className="px-4 py-2 rounded-lg bg-rose-600 text-white text-sm font-semibold hover:bg-rose-700 transition disabled:opacity-50 flex items-center gap-2"
                        >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <Panel className="bg-zinc-900/60 p-4 flex flex-col gap-3">
                        <div className="flex items-center gap-2">
                            ...
                            <h3 className="text-sm font-semibold text-zinc-200">Registro A</h3>
                        </div>
                    ...
                    <Panel className="bg-zinc-900/60 p-4 flex flex-col gap-3">
                        <div className="flex items-center gap-2">
                            ...
                            <h3 className="text-sm font-semibold text-zinc-200">Registro SRV</h3>
                        </div>
                        <div className="space-y-2 text-sm">
    ...
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.25 }} className="flex flex-col gap-6">
            <div className="space-y-3">
                <h2 className="text-lg font-semibold">Subdomínio</h2>
                <p className="text-sm text-zinc-400">Defina um subdomínio público para acessar seu servidor. Certifique-se que o domínio já está configurado pela administração.</p>
            </div>

            {error && (
                <div className="bg-rose-500/10 border border-rose-500/30 text-rose-300 p-3 rounded-md text-sm flex items-center gap-2">
                    <Icon name="alert-triangle" className="w-4 h-4" /> {error}
                </div>
            )}

            <form onSubmit={onSubmit} className="bg-zinc-900/50 rounded-lg p-5 flex flex-col gap-6">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="flex flex-col gap-2 md:col-span-1">
                        <label className="text-xs font-semibold uppercase tracking-wider text-zinc-400">Subdomínio</label>
                        <input
                            type="text"
                            value={subPart}
                        ...
                        />
                        <p className="text-[11px] text-zinc-500">Permitido: letras minúsculas, números e hífen.</p>
                    </div>

                    <div className="flex flex-col gap-2 md:col-span-1">
                        <label className="text-xs font-semibold uppercase tracking-wider text-zinc-400">Domínio</label>
                        <select
                        ...
                        <p className="text-[11px] text-zinc-500">Lista de domínios disponíveis.</p>
                    </div>

                    <div className="flex flex-col gap-2 md:col-span-1">
                        <label className="text-xs font-semibold uppercase tracking-wider text-zinc-400">Pré-visualização</label>
                ...
                <div className="flex justify-end">
                    <button
                        type="submit"
                        disabled={!canSubmit}
                        className="px-5 py-2.5 rounded-lg bg-purple-600 text-white text-sm font-semibold hover:bg-purple-700 transition-all duration-300 disabled:bg-zinc-700 disabled:cursor-not-allowed flex items-center gap-2"
                    >
                        {formLoading && <Icon name="loader" className="w-4 h-4 animate-spin" />}
