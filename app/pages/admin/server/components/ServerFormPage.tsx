...
const ServerFormPage: React.FC<ServerFormPageProps> = ({ server, onSave, isSubmitting, error, clearError }) => {
    ...
    const [activeTab, setActiveTab] = useState<'geral' | 'recursos' | 'alocacao' | 'configuracao'>('geral');
    ...
    useEffect(() => {
        if (server) {
            ...
            if (['geral','recursos','alocacao','configuracao'].includes(hash)) {
                setActiveTab(hash as any);
            }
            const onHashChange = () => {
                const h = window.location.hash.replace('#','');
                if (['geral','recursos','alocacao','configuracao'].includes(h)) setActiveTab(h as any);
            };
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            addToast(e.message || 'Falha ao deletar servidor', 'error');
        } finally {
            setIsDeleteModalOpen(false);
        }
    };

    // --- Renderizadores por aba (somente edição) ---
                            ...
                            {formData.additionalAllocationIds.map(allocId => {
                                ...
                                return (
                                    <div key={allocId} className="flex items-center justify-between bg-zinc-800/50 p-2 rounded-md text-sm">
                                        ...
                                        <button onClick={() => handleRemoveAllocation(allocId)} className="p-1 text-zinc-400 hover:text-rose-400 transition-colors"><Icon name="x" className="w-4 h-4" /></button>
                                    </div>
                                );
                            })}
                        </div>
                ...
                <div className="p-6 space-y-6">
                    <div>
                        ...
                        <CustomSelect
                            ...
                            disabled={!!server /* só bloquear em edição */}
                        />
                    </div>
                    {selectedCore && (
                        <>
    ...
    // ...existing code before return (keeping create mode rendering unchanged)...

    const isEditingMode = !!server; // simplifica a condição

    return (
        <>
            <header className="mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                <div>
                    <h1 className="text-4xl font-bold text-white">{isEditingMode ? `Editando Servidor` : 'Criar Novo Servidor'}</h1>
                    <p className="text-zinc-400 mt-1">{isEditingMode ? `Modificando "${formData.name}"` : 'Preencha os detalhes para configurar um novo servidor.'}</p>
                </div>
                {isEditingMode && server && (
                    <div className="flex flex-wrap gap-3">
                        <Link href={`/server/${server.id}`} className="px-4 py-2.5 rounded-lg bg-indigo-600 text-white text-sm font-semibold hover:bg-indigo-500 transition-colors flex items-center gap-2">
                            <Icon name="externalLink" className="w-4 h-4"/> Ver Servidor
                        </Link>
                        <button onClick={handleRequestDelete} className="px-4 py-2.5 rounded-lg bg-rose-600/90 text-white text-sm font-semibold hover:bg-rose-600 transition-colors flex items-center gap-2">
                            <Icon name="trash" className="w-4 h-4"/> Deletar
                        </button>
                    </div>
                )}
            </header>
            ...
            {!isEditingMode && (
                // Modo criação permanece monolítico (layout antigo) reutilizando as seções já existentes acima
                <div className="space-y-8">
                    {renderAbaGeral()}
                    {renderAbaRecursos()}
                    {renderAbaAlocacao()}
                    {renderAbaConfiguracao()}
                </div>
            )}

            {isEditingMode && (
                <>
                    <div className="flex border-b border-zinc-700/50 mb-8 overflow-x-auto no-scrollbar">
                        {([
                    ...
                    <div className="animate-[fadeIn_0.25s_ease]">
                        {activeTab === 'geral' && renderAbaGeral()}
                        {activeTab === 'recursos' && renderAbaRecursos()}
                        {activeTab === 'alocacao' && renderAbaAlocacao()}
                        {activeTab === 'configuracao' && renderAbaConfiguracao()}
                    </div>
                </>
            )}
            ...
            <ConfirmModal
                ...
                message="Tem certeza que deseja deletar este servidor? Esta ação não pode ser desfeita."
                icon="trash"
